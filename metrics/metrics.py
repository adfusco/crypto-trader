...
class Metrics:
    def __init__(self, portfolio_dict):
        ...
        if self.trade_history.empty:
            # a strategy/param combo may never trade over a given window
            self.trade_history = pd.DataFrame(columns=[
                'entry_timestamp', 'exit_timestamp', 'entry_price',
                'exit_price', 'qty', 'side', 'pnl',
            ])
        ...
        self.symbols = portfolio_dict['symbols']
        self.init_cash = portfolio_dict['init_cash']

        self.prices = portfolio_dict['benchmark_prices'].copy()
    ...
    def print_stats(self, start=None, end=None):
        ...
        for name, value in existing_stats.items():
            if name in ('max_drawdown_amt', 'max_drawdown_pct'):
                # both are {'amt', 'pct'} dicts (worst dd by $ vs by %); show flat
                print(f"{name}: {value['pct']:.2%} (${value['amt']:,.2f})")
            else:
                print(f'{name}: ' + str(value))
    ...
    def benchmark_curve(self):
        # equal-weight buy-and-hold basket: split init_cash across symbols,
        # each leg grows by its own close-price ratio.
        close_cols = [c for c in self.prices.columns if c.endswith('_close')]
        alloc = self.init_cash / len(close_cols)

        equity = None
        for col in close_cols:
            prices = self.prices[col]
            leg = alloc * (prices / prices.iloc[0])
            equity = leg if equity is None else equity + leg
        return equity

    def underwater_series(self):
        dd = pd.DataFrame(self.drawdowns)
        dd['timestamp'] = pd.to_datetime(dd['timestamp'])
        dd.set_index('timestamp', inplace=True)
        return -dd['drawdown_pct']

    def max_drawdown_window(self):
        dd = pd.DataFrame(self.drawdowns)
        dd['timestamp'] = pd.to_datetime(dd['timestamp'])
        dd.set_index('timestamp', inplace=True)

        trough_ts = dd['drawdown_pct'].idxmax()
        before_trough = dd.loc[:trough_ts]
        recovered = before_trough[before_trough['drawdown_pct'] == 0]
        peak_ts = recovered.index[-1] if len(recovered) else before_trough.index[0]
